...
class Block(BboxElement):
    ...
    @property
    def prelim_text(self):
        return "\n".join([line.prelim_text for line in self.lines])
    ...
    def filter_bad_span_types(
        self, bad_span_types: list = ["Page-footer", "Page-header", "Picture"]
    ):
        new_lines = []
        for line in self.lines:
            new_spans = []
            for span in line.spans:
                if self.block_type not in bad_span_types:
                    new_spans.append(span)
            line.spans = new_spans
            if len(new_spans) > 0:
                new_lines.append(line)
