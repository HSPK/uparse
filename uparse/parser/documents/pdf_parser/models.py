...
def setup_recognition_model(langs, device=None, dtype=None, checkpoint: str = "vikp/surya_rec"):
    if device:
        ...
        rec_model = load_recognition_model(langs=langs, checkpoint=checkpoint)
...
def setup_texify_model(device=None, dtype=None):
    if device:
        texify_model = load_texify_model(
            checkpoint=settings.TEXIFY_MODEL_NAME, device=device, dtype=dtype
        )
    else:
        texify_model = load_texify_model(
