...
from .schema import OrderBox, OrderResult
...
def batch_ordering(
    ...
    for i in tqdm(range(0, len(images), batch_size), desc="Finding reading order"):
        ...
        batch_bbox_mask = torch.from_numpy(np.array(batch_bbox_mask, dtype=np.int32)).to(
            model.device
        )
        batch_pixel_values = torch.tensor(np.array(batch_pixel_values), dtype=model.dtype).to(
            model.device
        )
        batch_bbox_counts = torch.tensor(np.array(batch_bbox_counts), dtype=torch.long).to(
            model.device
        )
