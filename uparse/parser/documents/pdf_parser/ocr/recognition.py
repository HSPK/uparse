...
from .surya_ocr import run_recognition
...
def run_ocr(
    ...
    langs: List[str],
    rec_model,
    ocr_method: Literal["surya", "tesseract"] = "surya",
    batch_size: int = 16,
    ...
    logger.info(f"[OCR] Method: {ocr_method}, Pages: {ocr_pages}")
    if ocr_method is None:
        ...
        new_pages = surya_recognition(doc, ocr_idxs, langs, rec_model, pages, batch_size=batch_size)
...
def surya_recognition(
    ...
    n_polygons = sum([len(p) for p in polygons])
    logger.info(f"Running recognition on {len(images)} pages, {n_polygons} polygons")
