...
from .surya_rec import batch_recognition
...
def run_recognition(
    ...
    rec_predictions = []
    for i in tqdm(range(0, len(all_slices), batch_size), desc="Recognizing Text"):
        batch_langs = all_langs[i : i + batch_size]
        batch_slices = all_slices[i : i + batch_size]
        batch_predictions, _ = batch_recognition(
            batch_slices, batch_langs, rec_model, rec_processor
        )
        rec_predictions.extend(batch_predictions)
