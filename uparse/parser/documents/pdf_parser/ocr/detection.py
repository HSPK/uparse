...
def batch_detection(
    images: List, model: EfficientViTForSemanticSegmentation, processor, batch_size=None
...
def batch_text_detection(
    ...
    logger.info(f"Running postprocessing with {max_workers} workers on {len(preds)} images")
...
def surya_detection(
    doc: PdfDocument,
    pages: List[Page],
    det_model: EfficientViTForSemanticSegmentation,
    batch_size: int = 16,
):
    ...
    logger.info(f"Detecting text bbox on {len(images)} pages")
    predictions = batch_text_detection(images, det_model, processor, batch_size=batch_size)
    return predictions
