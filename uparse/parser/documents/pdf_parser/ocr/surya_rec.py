...
def batch_recognition(images: List, languages: List[List[str]], model, processor, batch_size=None):
    ...
    for lang in languages:
        assert (
            len(lang) <= settings.RECOGNITION_MAX_LANGS
        ), f"OCR only supports up to {settings.RECOGNITION_MAX_LANGS} languages per image, you passed {lang}."
        ...
        if settings.RECOGNITION_STATIC_CACHE:
            ...
            batch_pixel_values = torch.cat(
                [
                    batch_pixel_values,
                    torch.zeros(
                        (batch_size - current_batch_size,) + batch_pixel_values.shape[1:],
                        dtype=model.dtype,
                        device=model.device,
                    ),
                ],
